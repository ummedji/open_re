$(function () {
    ...
    var distributor_comp_prd_validators = $("#distributor_compititor_product").validate({
        ignore: ".ignore",
        rules: {
            month_data:{
                required: true
    ...
    $("#add_row").click(function() {
        ...
        var $valid = $("#distributor_compititor_product").valid();
        if(!$valid) {
            distributor_comp_prd_validators.focusInvalid();
            return false;
        }
        else
        {
            ...
            /*var sku_ids = $('#distributor_comp_prd input[name^=prod_sku_id]').map(function(idx, elem) {
                return $(elem).val();
            }).get();
    ...
    $("#distributor_compititor_product").on("submit",function(){

        $('#prod_sku').addClass('ignore');
        $('#comp_prd').addClass('ignore');
        $('#qty').addClass('ignore');
        var param = $("#distributor_compititor_product").serializeArray();
        ...
        {
            if($("#distributor_comp_prd").children().length <= 0)
            {
                ...
                $.ajax({
                    type: 'POST',
                    url: site_url + "ecp/distributor_compititor_product_details",
                    data: param,
...
function add_row()
{
    ...
    var sr_no =$("#distributor_comp_prd > tr").length + 1;

    $("#distributor_comp_prd").append(
        "<tr>"+
        "<td data-title='Sr. No.' class='numeric'>" +
        "<input class='input_remove_border'  type='text' value='"+sr_no+"' readonly/>" +
        "</td>"+
        "<td  data-title='Action' class='numeric'>" +
        "<div class='delete_i distributor_comp_prd' attr-dele=''><a href='#'><i class='fa fa-trash-o' aria-hidden='true'></i></a></div>" +
        "</td>"+
...
$(document).on('click', 'div.distributor_comp_prd', function () {

    if (confirm("Are you sure?")) {
        $(this).closest('tr').remove();
    }
    var i = 1;
    $("tbody#distributor_comp_prd tr").each(function(  k, v  ) {
