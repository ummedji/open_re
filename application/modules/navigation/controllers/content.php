...
class content extends Admin_Controller {
    ...
    private function save_navigation($type = 'insert', $id = 0) {
        ...
        $navigation_table = $this->db->dbprefix("navigation");

        $this->form_validation->set_rules('title', 'Title', 'required|trim|xss_clean|max_length[255]');
        if ($type == "insert") {
            $this->form_validation->set_rules('position', 'Position', "required|trim|xss_clean|max_length[255]|is_unique[{$navigation_table}.position]");
        } else {
