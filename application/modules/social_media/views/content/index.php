<?php if (!isset($ajax)) : ?>
    <div class="admin-box">
        ...
        <div class='grid-filters'>
            ...
            <table>
                <tr>
                    <td><select name='category' class='category-dropdown reset-dropdown form-control input-medium' ><option value='all'>All</option><option value='active'>Active</option><option value='inactive'>Inactive</option></select></td>
                    <td>&nbsp;</td>
                    <td></td>
        ...
        <table class="table table-striped table-bordered table-hover dataTable no-footer">
            <thead>
                <tr>
                    <?php if ($this->auth->has_permission('Social_Media.Content.Delete') && isset($records) && is_array($records) && count($records)) : ?>
            ...
            <?php if (isset($records) && is_array($records) && count($records)) : ?>
                ...
                <?php if ($this->auth->has_permission('Social_Media.Content.Delete')) : ?>
                    ...
                    <?php echo lang('bf_with_selected') ?>
                                            <input type="submit" name="delete" id="delete-me" class="btn btn-danger" value="<?php echo lang('bf_action_delete') ?>" onclick="return confirm('<?php echo lang('social_media_delete_confirm'); ?>')">
                                    </td>
                            </tr>
                <?php endif; ?>
                </tfoot-->
                ...
                <?php if (isset($records) && is_array($records) && count($records)) : ?>
                    <?php foreach ($records as $record) : ?>
                        <tr>
                            ...
                            <td><?php
                                $socialImagepath = FCPATH . "/assets/uploads/socialmedia/" . $record->image;
                                if (file_exists($socialImagepath)) {
                                    $socialImage = base_url() . "/assets/uploads/socialmedia/" . $record->image;
                                } else {
                                    $socialImage = base_url() . "assets/images/no_image.jpg";
                                }
                                ?><img src="<?php echo $socialImage ?>" height="27" width="27"></td>
                            <td class="position">
                            ...
                            <?php if ($this->auth->has_permission('Social_Media.Content.Edit')) : ?>
                                <?php echo anchor(SITE_AREA . '/content/social_media/edit/' . $record->id, '<i class="glyphicon glyphicon-edit">&nbsp;</i>') ?>&nbsp;&nbsp;
                                <?php endif; ?>
                                <?php if ($this->auth->has_permission('Social_Media.Content.Delete') && isset($records) && is_array($records) && count($records)) : ?>            
