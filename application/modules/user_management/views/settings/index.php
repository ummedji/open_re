    ...
    <div class='grid-filters'>
        ...
        <table>
            <tr>
                ...
                <td>
                    <?php if ($this->auth->has_permission("{$module_config['module_permission_name']}.Settings.Delete")) : ?>
...
<?php endif; ?>
    <table class="table table-striped table-bordered table-hover dataTable no-footer">
        <thead>
        <tr>
            <?php if ($this->auth->has_permission("{$module_config['module_permission_name']}.Settings.Delete") && isset($records) && is_array($records) && count($records)) : ?>
            ...
            <th>
                Username
                <i class="icon-arrow-up sort" rel="asc" for="username"></i>
                <i class="icon-arrow-down sort" rel="desc" for="username"></i>
            </th>
            <th>
                Display Name
            ...
            <!--th>role_id</th>
            <th>Email                        <i class="icon-arrow-up sort" rel="asc" for="email"></i>
                <i class="icon-arrow-down sort" rel="desc" for="email"></i></th>
            <th>Username                        <i class="icon-arrow-up sort" rel="asc" for="username"></i>
                <i class="icon-arrow-down sort" rel="desc" for="username"></i></th>
            <th>password_hash</th>
            <th>reset_hash</th>
            <th>salt</th>
            <th>last_login</th>
            ...
            <th>ban_message</th>
            <th>reset_by</th>
            <th>display_name                        <i class="icon-arrow-up sort" rel="asc" for="display_name"></i>
                <i class="icon-arrow-down sort" rel="desc" for="display_name"></i></th>
            <th>display_name_changed</th>
            <th>timezone</th>
            <th>language</th>
            <th>activate_hash</th>
            <th>Created</th>
            ...
            <!--tfoot>
                <?php if ($this->auth->has_permission("{$module_config['module_permission_name']}.Settings.Delete")) : ?>
                    ...
                    <?php echo lang('bf_with_selected') ?>
                                                                                                <input type="submit" name="delete" id="delete-me" class="btn btn-danger" value="<?php echo lang('bf_action_delete') ?>" onclick="return confirm('<?php echo lang('user_management_delete_confirm'); ?>')">
                                                                                        </td>
                                                                                </tr>
                <?php endif; ?>
                </tfoot-->
        ...
        <?php if (isset($records) && is_array($records) && count($records)) : ?>
            <?php foreach ($records as $record) : ?>
                <tr>
                    ...
                    <td>
                        <a href="<?php echo site_url(SITE_AREA . "/settings/{$module_config['module_name']}/edit/" . $record->id); ?>"><?php echo $record->username; ?></a>
                        <?php if ($record->banned) echo '<span class="badge badge-warning">Banned</span>'; ?>
                    </td>
                    ...
                    ?>
                    <td>
                        <span style="cursor: pointer;" class="badge badge-<?php echo $class; ?> toggle_status"
                              rel_id="<?php echo $record->id; ?>"><?php echo $status ?></span>
                    </td>

                    <td>
                        <?php if ($this->auth->has_permission("{$module_config['module_permission_name']}.Settings.Edit")) : ?>
                            <?php echo anchor(SITE_AREA . "/settings/{$module_config['module_name']}/edit/" . $record->id, '<i class="glyphicon glyphicon-edit">&nbsp;</i>') ?>&nbsp;&nbsp;
                        <?php endif; ?>
