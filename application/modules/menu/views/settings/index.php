<?php if (!isset($ajax)) : ?>
    ...
    <div class='grid-filters'>
        ...
        <table>
            <tr>
                <td><select name='category' class='category-dropdown reset-dropdown'>
                        <option value='all'>All</option>
                        <option value='active'>Active</option>
                        <option value='inactive'>Inactive</option>
                    </select></td>
                <td><input type='text' class='search-field reset-input' rel_id='serach_filed1' name='search[title]'/>&nbsp;&nbsp;<select
                        class='search-field-dropdown reset-dropdown' rel='serach_filed1'>
                        <option value='title'>Title</option>
                        <option value='alias'>Alias</option>
                        <option value='navigation_id'>Navigation</option>
                    </select>&nbsp;&nbsp;</td>
                <td></td>
                <td>
...
<?php endif; ?>
    <table class="table table-striped table-bordered table-hover dataTable no-footer">
        <thead>
        <tr>
            <?php if ($this->auth->has_permission('Menu.Settings.Delete') && isset($records) && is_array($records) && count($records)) : ?>
                <th class="column-check"><input class="check-all" type="checkbox"/></th>
            <?php endif; ?>

            <th>Title <i class="icon-arrow-up sort" rel="asc" for="title"></i>
                <i class="icon-arrow-down sort" rel="desc" for="title"></i></th>
            <th>Alias <i class="icon-arrow-up sort" rel="asc" for="alias"></i>
                <i class="icon-arrow-down sort" rel="desc" for="alias"></i></th>
            <th>Link</th>
            <th>Parent</th>
            <th>Navigation <i class="icon-arrow-up sort" rel="asc" for="navigation_id"></i>
                <i class="icon-arrow-down sort" rel="desc" for="navigation_id"></i></th>
            <th>Target Window</th>
            <th>Image</th>
            <th>Access</th>
            <th>Meta title</th>
        ...
        <?php if (isset($records) && is_array($records) && count($records)) : ?>
            <?php foreach ($records as $record) : ?>
                <tr>
                    ...
                    <td><?php echo $record->navigation_id ?></td>
                    <td><?php echo $record->window ?></td>
                    <td><?php echo $record->image_name ?></td>
                    <td><?php echo $record->access_role_id ?></td>
                    ...
                    <td class="position">
                        <?php if (isset($max_position) && !empty($max_position)) { ?>
                            <?php if ($record->position < $max_position) { ?>
                                &nbsp;<i rel_id="<?php echo $record->id ?>" position="<?php echo $record->position ?>"
                                         state="up" class="icon-chevron-down"></i>
                            <?php } ?>
                        <?php } ?>
                        <?php if (isset($min_position) && !empty($min_position)) { ?>
                            <?php if ($record->position > $min_position) { ?>
                                &nbsp;<i rel_id="<?php echo $record->id ?>" position="<?php echo $record->position ?>"
                                         state="down" class="icon-chevron-up"></i>
                            <?php } ?>
                        <?php } ?>
                    </td>                <?php
                    if ($record->status == 1) {
                    ...
                    <td>
                        <?php if ($this->auth->has_permission('Menu.Settings.Edit')) : ?>
                            <?php echo anchor(SITE_AREA . '/settings/menu/edit/' . $record->id, '<i class="icon-note">&nbsp;</i>') ?>&nbsp;&nbsp;
                        <?php endif; ?>
                        <?php if ($this->auth->has_permission('Menu.Settings.Delete') && isset($records) && is_array($records) && count($records)) : ?>
