...
<div class='admin-box portlet box green'>
 <div class='portlet-title'>
        <div class='caption'>
            <i class='fa fa-pencil'></i> Edit Country Master
        </div>
    </div>
	...
	<?php echo form_open($this->uri->uri_string(), 'class="form-horizontal"'); ?>
		...
		<fieldset class='form-actions'>
			<input type='submit' name='save' class='btn green' value="<?php echo lang('country_master_action_edit'); ?>" />
			<?php echo lang('bf_or'); ?>
			<?php echo anchor(SITE_AREA . '/master/country_master', lang('country_master_cancel'), 'class="btn default"'); ?>
			
			<?php if ($this->auth->has_permission('Country_Master.Master.Delete')) : ?>
				<?php echo lang('bf_or'); ?>
				<button type='submit' name='delete' formnovalidate class='btn btn-danger' id='delete-me' onclick="return confirm('<?php e(js_escape(lang('country_master_delete_confirm'))); ?>');">
					<span class='icon-trash icon-white'></span>&nbsp;<?php echo lang('country_master_delete_record'); ?>
				</button>
			<?php endif; ?>
		</fieldset>
    <?php echo form_close(); ?>
